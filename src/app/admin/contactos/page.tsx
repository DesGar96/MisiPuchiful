...
import { Container, Row, Col, Card, Badge, Spinner, Alert, Table, Form } from 'react-bootstrap';
...
export default function AdminContactosPage() {
  ...
  const marcarComoLeido = async (id, leido) => {
    try {
      const response = await fetch('/api/contacto/admin', {
        method: 'PATCH',
    ...
    <div style={{ backgroundColor: '#F8F6F2', minHeight: '100vh', padding: '3rem 0' }}>
      <Container fluid className="px-5">
        ...
        <div style={{ 
          ...
          <h1 className="display-4 fw-bold mb-2" style={{ color: '#2E7D32' }}>
          ...
          <Card.Body className="p-4">
            {contactos.length === 0 ? (
              <div className="text-center py-5">
                ...
                <h4 className="mt-3" style={{ color: '#2E7D32' }}>No hay mensajes</h4>
              </div>
            ) : (
              <Table responsive hover style={{ color: '#6B5E4A' }}>
                ...
                <tbody>
                  {contactos.map((contacto) => (
                    <tr key={contacto.id} style={{
                      backgroundColor: contacto.leido ? 'white' : '#FFF3E0',
                      fontWeight: contacto.leido ? 'normal' : 'bold'
                    }}>
                      <td>
                        <Badge style={{ 
                          backgroundColor: contacto.leido ? '#A8E6CF' : '#FFD3B6',
                          color: '#2E7D32',
                          padding: '0.5rem 1rem',
                          borderRadius: '30px'
                        }}>
                      ...
                      <td>{formatDate(contacto.fecha_creacion)}</td>
                      <td>{contacto.nombre}</td>
                      <td>
                        <a href={`mailto:${contacto.email}`} style={{ color: '#2E7D32', textDecoration: 'none' }}>
                          {contacto.email}
                        </a>
                      </td>
                      <td>
                        {contacto.telefono && (
                          <a href={`tel:${contacto.telefono}`} style={{ color: '#2E7D32', textDecoration: 'none' }}>
                            {contacto.telefono}
                          </a>
                        )}
                      </td>
                      ...
                      <td>
                        <Form.Check
                          ...
                          onChange={() => marcarComoLeido(contacto.id, contacto.leido)}
                          style={{ color: '#2E7D32' }}
                        />
                      </td>
                    </tr>
